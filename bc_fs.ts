...
if (document.body['offsetWidth'] < 1250) fullscreen();

setOverlay();
window.addEventListener("resize", function() {
  if (fullscr) resize_canvas;
  setOverlay();
});
...
window.onload = function() {
  ...
  game_over_menu = document.getElementById("game_over");
...
function levels(menu) {
  menu.classList.remove("buttons_in");
  levels_menu.classList.add("buttons_in");
}
...
function lvl(lvl, menu) {
  menu.classList.remove("buttons_in");
  hide_panels();
  init();
  level_running = parseInt(lvl);
}
...
function game_over() {
  ...
  clearInterval(game_timer);
  show_panels(true);
  game_over_menu.classList.add("buttons_in");
}
...
var Bubble = function(id, x, y, rgb) {
    ...
    this.update = function() {
      ...
      if((this.y + this.diffY < this.radius) || (this.y + this.diffY > height-this.radius))
        ...
        if (del_bubble) {
          if (mills%200 == 0) {
            if (this.diffX > 1) this.diffX--;
            if (this.diffY > 1) this.diffY--;
        }
...
function draw() {
  ...
  function drawBlack(b) {
    if (del_alpha <= 0.7) del_alpha += 0.1;
  ...
  for (var i = 0; i < bubbles.length; i++) {
    ...
    if (del_bubble) {
      if (bubbles[i] == del_bubble) continue;
      else {
        bubbles[i].fade(3);
      }
    }
...
function getBubble(event) {
  ...
  for (var i=0; i<bubbles.length; i++) {
    var b = bubbles[i];
    if ((Math.abs(b.x - mX) > maxrad) || (Math.abs(b.y - mY) > maxrad)) continue;
    else {
